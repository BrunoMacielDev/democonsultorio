...
const InterResetPro = () => {
  ...
  const handleOnClick = async (e) => {
    e.preventDefault();
    if (email) {
      const response = await API.getProfesionalesByEmail({ email });
      if (response.status) {
        window.location.replace(
          `/reset-password/profesionales/${response.data[0].id_pro}`
        );
      } else {
        toast({
      ...
      <div className="container__resetPro-card">
        <div className="container__resetPro-card_column">
          ...
          <form>
            ...
            <div className="flex">
              <button onClick={handleOnClick}>Continuar
              </button>
